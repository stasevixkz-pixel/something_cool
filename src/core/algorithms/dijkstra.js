 ...
 * @returns {Generator<VisitResult | CompleteResult | ErrorResult>} Generator yielding Dijkstra steps.
 */
export function dijkstraGenerator(grid, start, end) {
    // Input validation
    ...
    // Priority queue implementation using array + sort by g
    // Each element: { x, y, g }
    const priorityQueue = [{ x: start.x, y: start.y, g: 0 }];
    
    // Track minimum g-score for each position
    const minG = new Map();
    ...
    while (priorityQueue.length > 0) {
        // Sort by g (ascending) and pop the first element (minimum g)
        priorityQueue.sort((a, b) => a.g - b.g);
        ...
        for (const { dx, dy } of directions) {
            ...
            // Calculate new g-score (default weight = 1)
            ...
            if (!minG.has(neighborKey) || newG < minG.get(neighborKey)) {
                ...
                priorityQueue.push({ x: nx, y: ny, g: newG });
            }
        }
    }
